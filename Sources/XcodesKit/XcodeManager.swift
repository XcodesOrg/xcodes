...
extension XcodeManager {
    ...
    private static let configuration = applicationSupportPath/"configuration.json"
    ...
    private func loadConfiguration() throws {
        let data = try Data(contentsOf: XcodeManager.configuration.url)
    ...
    private func saveConfiguration(_ configuration: Configuration) throws {
        ...
        try FileManager.default.createDirectory(at: XcodeManager.configuration.url.deletingLastPathComponent(),
                                                withIntermediateDirectories: true)
        try data.write(to: XcodeManager.configuration.url)
    }
}
